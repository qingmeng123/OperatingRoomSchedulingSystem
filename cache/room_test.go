...
func TestOperatingRoomFunctions(t *testing.T) {
	InitCache()
	// 添加手术室信息
	rooms := []model.OperatingRoom{
		{Id: 1, Name: "Room 1", Category: 1},
		{Id: 2, Name: "Room 2", Category: 2},
		{Id: 3, Name: "Room 3", Category: 1},
	}
	//预约手术
	startTime := time.Date(2024, 5, 9, 8, 0, 0, 0, time.UTC) // 模拟开始时间
	endTime := time.Date(2024, 5, 9, 10, 0, 0, 0, time.UTC)  // 模拟结束时间
	err := ReserveOperatingRoom(1, startTime, endTime)
	if err != nil {
